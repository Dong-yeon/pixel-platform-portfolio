...
### 처리량: 구간별(leg) 예약 — 다음 개선 과제

동시 주행이 평균 1~2대에 머문다. 원인을 측정으로 좁혔다.

배차 시 **"현재 위치 → 픽업 → 하역"을 통째로 한 번에 예약**하는데, 로봇이 픽업 지점에서
멀리 있으면 이 경로가 공장을 가로지른다. 실측 로그:

```
robot AMR-01 cannot take T-286789 — segments busy
  [V:3:top, V:3:mid, AL:3-11, AL:11-18, V:18:bot, AL:18-25, AL:25-32, AL:32-41, V:41:mid]
```
9개 구간(사실상 전 구역)을 한 번에 요구한다. 정작 주행 중인 로봇은 3개만 쥐고 있다
(점진 반납은 잘 동작한다).

**해결 방향** — 픽업까지(leg1)와 픽업→하역(leg2)을 나눠 예약한다.
1. 배차 시 leg1만 예약하고 GOTO도 leg1 웨이포인트만 보낸다
2. 로봇이 픽업 도착을 보고하면 그때 leg2를 예약하고 두 번째 GOTO를 보낸다
3. leg2 예약에 실패하면 픽업 지점에서 대기한다

로봇이 한 번에 쥐는 구간이 절반으로 줄어 동시 주행이 크게 는다.
확보는 여전히 leg 단위 일괄이므로 교착 안전성은 유지된다.
다만 픽업 도착 이벤트와 2단계 GOTO 프로토콜이 필요하다.
