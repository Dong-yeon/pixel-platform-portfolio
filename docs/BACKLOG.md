# 플랫폼 백로그

범위 밖 아이디어를 여기에 모은다. 구현 전에 먼저 기록한다.
(모듈별 백로그는 각 모듈의 `docs/BACKLOG.md`)

## 통합 공장 지도 ✅ 1차 구현 완료 (대시보드 매핑 방식)

**통합 현황 탭에서 PixelFactory와 PixelFleet을 하나의 공장 평면도로 보여준다.**

지금은 두 모듈이 KPI 카드와 이벤트 타임라인으로만 합쳐져 있고, 지도는 PixelFleet
탭에만 있다. 같은 평면도 위에 설비와 AMR을 함께 그리면 "하나의 공장을 두 시스템이
관제한다"는 플랫폼의 요지가 한 화면에 드러난다.

- **설비** — 사각형, 상태별 색(RUNNING 초록 / IDLE 회색 / DOWN 빨강 / QUALITY_HOLD 주황)
- **AMR** — 원형, 실시간 이동 (지금 구현 그대로)
- **작업 흐름** — 진행 중인 운송 작업의 출발→도착을 선으로 연결하면 물류 흐름이 보인다

### 필요한 것

1. **설비에 좌표 부여** — 현재 `Equipment`에는 위치가 없다. 선택지:
   - (a) `equipments` 테이블에 `pos_x`, `pos_y` 컬럼 추가 (마이그레이션 필요)
   - (b) 대시보드/게이트웨이 쪽 레지스트리로만 매핑 (DB 변경 없음, 빠름)
2. **좌표계 통일** — fleet의 44×24 평면도를 플랫폼 표준으로 삼는다. 지금 노드 좌표가
   `control-service/LocationRegistry`, `robot-sim/NodeMap`, `dashboard/types.ts` 세 곳에
   중복돼 있는데(아래 항목), 통합 지도를 만들면 단일화 필요성이 더 커진다.
3. **설비와 노드의 관계 정의** — STATION-A가 곧 CNC-01이 있는 자리인지, 인접한 적재
   지점인지. 서사를 정하면 좌표 배치가 자연스러워진다.

> **(a) 정식화까지 완료 (P11).** `equipments.pos_x/pos_y` + `layout_nodes` + `layout_settings`.
> 설비를 늘리거나 옮겨도 대시보드를 고칠 필요가 없다(실측: DB에서 좌표만 바꿔 지도가 이동).

## 좌표 정의 단일화 ✅ 완료 (P11)

**pixel-factory가 평면도 마스터를 소유한다.** 평면도는 공장의 것이지 물류만의 것이 아니고,
설비·하역 지점·(P12의) POP 단말이 모두 같은 바닥 위에 있다.

| 소비처 | 방식 |
|---|---|
| 대시보드 | `GET /api/factory/layout` + 설비 좌표는 `Equipment`에 실려 온다. **하드코딩 좌표 0** |
| control-service `LocationRegistry` | 기동 시 + 5분 주기로 factory에서 받아 캐시. 실패 시 하드코딩 폴백 + WARN |
| robot-sim `NodeMap` | **그대로 둔다.** 시뮬레이터는 물리 세계를 흉내내는 쪽이라 서버가 알려주는 대로 위치를 바꾸면 안 되고, 서버가 죽어도 돌아야 한다. 대신 `NodeMapLayoutConsistencyTest`가 마이그레이션 SQL과 대조해 **어긋나면 빌드를 깨뜨린다**(좌표 하나를 틀리게 만들어 실제로 실패하는지 확인했다) |

### 남은 것 — `LaneGraph`의 통로 y·연결로 x

`traffic/LaneGraph`는 통로 y와 세로 연결로 x를 `static final`로 갖고 있고, 그 값으로 구간
이름(`AU:11-18` 등)까지 만든다. 동적으로 바꾸려면 교통 통제 전체를 건드려야 해서 P11 범위에
넣지 않았다. 대신 `LocationRegistry`가 서버 평면도를 받을 때 **통로 y가 다르면 ERROR 로그**를
남긴다 — 조용히 어긋나면 그린 선과 실제 주행이 갈리고 구간 점유가 엉킨다.

연결로 x는 아직 어디에도 마스터가 없다(레인망은 factory가 모르는 물류 개념이다).
fleet이 소유하는 게 맞을 수 있는데, 그러면 "평면도는 factory 것"과 경계가 미묘해진다 — 결정 필요.

### 서비스 간 인증(M2M)

`GET /api/layout`은 **인증 없이 열려 있다.** 평면도는 민감정보가 아니고(설비 위치·하역 좌표뿐),
fleet이 기동 시 읽어야 하는데 서비스 간 인증이 없어 토큰을 받을 방법이 없다. 그래서 fleet은
게이트웨이를 경유하지 않고 모듈에 직접 붙는다. M2M 인증이 생기면 이 엔드포인트를 닫고
서비스 토큰으로 게이트웨이를 경유하게 바꾼다.


## AMR 교통정리 ✅ 구현 완료 (전체 경로 예약 방식)

### "겹친다"는 세 가지 다른 문제였다

증상이 비슷해 보여 한 문제로 착각하기 쉬운데, 원인도 해법도 각각 다르다.

| 겹침 | 원인 | 해법 | 위치 |
|---|---|---|---|
| **정차 겹침** | 같은 노드를 고른 로봇들이 노드 좌표 그 점에 정확히 겹쳐 선다 | 로봇 인덱스로 노드 주변 반경 1.1에 원형 정차 자리 분배(결정적이라 재현 가능) | robot-sim `Simulator#spot` |
| **경로 겹침 / 가로지르기** | 두 점을 직선으로 이으면 대각선이 설비·스테이션을 관통한다. 실제 AMR은 그렇게 못 간다 | 레인망(가로 통로 2 + 세로 연결로 6)으로만 경로를 만든다. **대시보드도 같은 규칙으로 폴리라인을 그려** 화면과 실제 주행을 일치시킨다 | `traffic/LaneGraph`, dashboard `types.ts#routePoints` |
| **주행 중 충돌** | 두 대가 같은 구간에 동시 진입 | 구간 배타 점유 + leg 단위 예약 + 점진 반납 (아래) | `traffic/TrafficController` |

세 번째를 풀어도 첫 번째·두 번째는 남는다. 구간 점유는 **주행 중** 충돌만 막고,
정차 지점과 화면 표현은 통제하지 않기 때문이다.

**서버가 경로를 소유하고 구간 점유권을 관리한다.**

- 서버가 레인 그래프(`traffic/LaneGraph`)로 경로를 계산해 웨이포인트를 로봇에 지시한다.
  로봇은 스스로 길을 정하지 않는다 — 그래야 서버가 통제할 수 있다.
- `traffic/TrafficController`가 구간 점유를 관리한다. 지나갈 구간을 **전부 확보해야만**
  배차하고, 하나라도 막혀 있으면 작업은 PENDING으로 남아 다음 패스에서 재시도한다.
- 작업 완료·실패 시 점유를 해제한다.
- 점유 현황은 `GET /api/traffic/reservations`로 볼 수 있다.

### 고아 작업 복구 ✅

로봇이 지시를 놓치면(배포 중 구독 전 발행 등) 그 작업이 ASSIGNED로 굳어 **레인 구간을
영원히 점유**하고 함대 전체를 멈춘다 — 실제 배포에서 발생했다.
`StuckTaskWatchdog`이 배정 ACK 타임아웃(30초)과 주행 타임아웃(5분)을 감시해 실패 처리하고,
실패 경로가 점유 해제와 재큐잉을 수행한다.

### 구간별(leg) 예약 ✅ 구현 완료

배차 시 leg1(현재→픽업)만 예약하고, 로봇이 픽업 도착을 보고하면 그때 leg2(픽업→하역)를
예약해 두 번째 GOTO를 보낸다. 로봇이 한 번에 쥐는 구간이 절반으로 줄었다.

구현 중 실제로 터진 것 두 가지 — 둘 다 이 구조의 핵심이라 기록해 둔다.

1. **교착(deadlock)** — 처음엔 leg1을 쥔 채 leg2를 요청했다. 이게 hold-and-wait라
   로봇 4대가 서로가 쥔 구간을 기다리며 픽업에서 전부 멈췄다(완료 0건).
   → 픽업 도착 시 **leg1을 전부 놓은 뒤** leg2를 요청한다. 픽업 자리는 레인 밖이라
   놓아도 통행을 막지 않는다.
2. **leg2 오배달** — 작업은 배차 직후 IN_PROGRESS가 되므로, 상태만 보고 leg2를 보내면
   아직 leg1을 달리는 로봇에게 도착한다. 로봇은 "이미 작업 중"으로 거부하고 서버는
   보냈다고 여겨, 구간만 잡힌 채 아무도 안 움직였다.
   → 픽업 도착을 **실제로 보고한** 작업만 leg2 대상으로 삼는다.

검증: leg2 전달 9회, GOTO 거부 0회, 작업 지속 완료(처리량 약 2건/분).

### 남은 처리량 개선

실제 동시 이동은 여전히 1~2대다. 로봇 6대가 모두 작업을 배정받은 상태이므로 공급이 아니라
**레인 용량**이 한계다. 다음 지렛대는 **구간을 더 잘게 쪼개는 것** — 지금 가로 구간은
연결로 사이(7칸)가 통째로 하나여서, 같은 구간에 있는 한 뒤차가 못 들어온다.
연결로를 늘리거나 구간을 절반으로 나누면 로봇들이 줄지어 따라갈 수 있다.

> **관측 가능해짐(2026-08-13):** `fleet_traffic_segments_occupied`(Prometheus)가 지금
> 이 순간 점유된 구간 수를 실시간으로 보여준다. 이 항목에 실제로 손댈 때, 손대기
> 전후로 이 값의 시계열을 비교하면 "구간을 쪼갠 게 실제로 동시 주행을 늘렸는지"를
> 감으로 말하지 않고 숫자로 말할 수 있다.

- **충전 주행은 통제 밖** — 저배터리 복귀는 여전히 로봇이 스스로 결정한다.
  서버가 충전도 작업으로 만들어 배차하면 완전해진다.

### 🐛 배터리 20~24% 사각지대 — 함대가 영구히 멈췄다 ✅ 수정됨

P6 검증 중 발견. 세 값이 서로 안 맞아 로봇이 빠져나올 수 없는 구간이 생겼다.

| 값 | 위치 |
|---|---|
| 배차 최소 배터리 **25%** | `NearestBatteryAwareAssignmentPolicy.MIN_BATTERY_PERCENT` |
| 충전 시작 기준 **20%** ← 문제 | robot-sim `sim.low-battery-threshold` |
| 배터리는 **MOVING일 때만** 닳음 | robot-sim `Simulator.tickMoving` (`tickIdle`은 안 닳는다) |

작업을 20~24%에서 끝낸 로봇은 **배차받기엔 낮고, 충전 가려면 높고, 가만히 있으면
더 닳지도 않는다.** 그 상태로 영원히 IDLE이다. 로봇이 하나씩 이 구간에 빠지면서
결국 함대 전체가 멈췄다(실제로 6대 전부 20~23%에서 멈춰 있었다).

교통 교착과 증상이 똑같아 보이지만(전원 IDLE, 완료 0건) 원인이 전혀 다르다.
점유 현황(`GET /api/traffic/reservations`)이 **비어 있으면** 교통 문제가 아니라 이쪽이다.

**수정:** 충전 기준을 20 → **30**으로 올려 배차 기준(25)보다 높게 만들었다.
불변식 **"충전 복귀 기준 > 배차 최소 배터리"**를 양쪽 코드 주석에 박아 뒀다.

검증: 배터리가 30% 아래로 내려간 로봇이 사각지대에 머무르지 않고 충전으로 전환돼
복귀하는 것을 실측했다(AMR-03 → CHARGING → 96% → 작업 재개).

> **참고 — 0%로 돌아다니는 로봇은 멈춘 게 아니다.** 충전 판단은 `tickIdle`에서만 하므로
> 작업 수행 중에는 배터리와 무관하게 계속 간다. 작업이 끝나야 도크로 출발하고, 그 주행
> 중에도 계속 닳아 0%로 클램프될 수 있다. 도착하면 충전한다(실측: AMR-05 0% → 도크 도착 →
> 48%). 배차는 25% 미만 로봇에 새 작업을 주지 않으므로 결국 항상 충전으로 수렴한다.
> 현실성을 높이려면 저배터리 시 작업을 중단하고 복귀해야 하는데, 그건 "충전도 서버가
> 배차하는 작업으로" 만드는 아래 항목과 함께 풀 문제다.

근본 원인은 두 값이 **서로 다른 서비스**에 나뉘어 있다는 것이다. 아래 "충전 주행은 통제 밖"을
해결해 충전도 서버가 배차하는 작업으로 만들면 애초에 어긋날 수가 없어진다.

## 게이트웨이 중앙 인증 ✅ 구현 완료 (P6)

**발급은 모듈이, 검증은 게이트웨이가.** 게이트웨이는 사용자 저장소가 없어 토큰을 발급할 수
없으므로 로그인은 신원 담당 모듈(`AUTH_MODULE_URI`, 현재 pixel-factory)에 위임하고,
이후 모든 요청은 `AuthenticationGlobalFilter`가 관문에서 검증한다.

- 세 서비스가 **같은 `PLATFORM_JWT_SECRET`**을 쓴다 → 로그인 한 번, 토큰 하나로 전부 통한다.
  (이전에는 모듈마다 키가 달라 대시보드가 모듈마다 로그인하고 토큰을 2개 들고 있었다.)
- 통과한 요청에 `X-Auth-User`/`X-Auth-Role`을 붙이고, **클라이언트가 보낸 `X-Auth-*`는 항상 지운다.**
  이걸 빠뜨리면 헤더만 붙여 관리자로 행세할 수 있다 — 헤더 기반 신원 전달의 대표적 구멍이다.
- 모듈도 자체 JWT 필터를 유지한다(방어 심층). 9001/9002에 직접 붙어도 인증이 필요하다.

게이트웨이는 `shared`(공통 코어)를 의존할 수 없다 — `spring-boot-starter-web`(서블릿)을
api로 노출해서 WebFlux 게이트웨이가 서블릿 스택으로 떠 버린다. 그래서 검증에 필요한 만큼만
jjwt를 직접 가져왔다.

### 남은 것

- **`/ws/**`는 여전히 인증 없이 통과한다.** SockJS 핸드셰이크에 Authorization 헤더를 실을 수
  없어서다. 토큰을 STOMP CONNECT 프레임에 담아 검증하면 막을 수 있다.
- **권한(Role) 기반 접근 제어는 모듈 몫.** 게이트웨이는 "인증됐는가"만 본다.
- **사용자 저장소가 모듈마다 따로 있다**(`users` 테이블 2벌). 전용 인증 서비스로 분리하면
  `AUTH_MODULE_URI`만 그쪽으로 돌리면 된다 — 지금 구조가 그 갈아끼움을 전제로 짜여 있다.
- **리프레시 토큰 없음.** 2시간 만료 시 다시 로그인해야 한다.

## 그 외

- ~~**PixelFactory 실시간 push**~~ → **P10에서 완료.** `/ws/factory`로 설비·이벤트 즉시 push,
  OEE는 5초 주기. 게이트웨이 WS 라우트도 모듈별로 갈랐다(`/ws/fleet`, `/ws/factory`).
- **OEE 계산 엔진** — 가동률×성능×품질. PixelFactory 로드맵 Phase 2가 미완이라 대시보드에
  품질률만 나온다.
- ~~**이벤트 보존 정책** — `fleet_events`/`factory_events`가 무한 증가. 배포 환경 비용 직결.~~
  → **✅ 완료(2026-08-13).** 두 서비스 모두 `@Modifying @Query` 벌크 DELETE로 매일 새벽
  오래된 이벤트를 지운다(기본 90일, `FACTORY_EVENT_RETENTION_DAYS`/`FLEET_EVENT_RETENTION_DAYS`로
  조정). 파생 쿼리 대신 벌크 SQL을 쓴 이유는 `FactoryEventRetentionJob` Javadoc 참고 —
  수만 건을 엔티티로 읽어와 한 건씩 지우는 방식은 이 테이블 규모에서 그 자체가 부하다.
  아카이브가 아니라 삭제인 이유: OEE·화면이 항상 최근 구간만 조회하고, 오래된 원본을
  재집계하는 화면·배치가 지금 없다.
- **CI** — GitHub Actions로 모듈별 빌드/타입체크 (경로 필터로 변경된 모듈만).
- **P6 게이트웨이 중앙 인증** — 지금은 모듈이 각자 JWT 검증, 대시보드가 토큰 2개 보관.

## 2026-07 기술뉴스에서 도출한 항목 (P16 ~ P18)

근거: 2026-07-30~31 국내 언론 다수 기사 분석(비공개 리서치 문서, 공개 버전에서 제외).
**기사 자체를 플랫폼에 넣지 않는다.** 뉴스는 근거이고, 여기 적힌 것만 구현 대상이다.

### P16. 인증 경계 정리 (보안 하드닝) ✅ 완료(WP0 CORS + WP1 STOMP 인증 + WP2 M2M + WP3 MQTT, 2026-09-02)

> P15 배포 단계에 몰아둔 보안 항목을 앞당긴다. **근거가 생겼다** — IBM '2026 데이터 유출
> 비용 보고서'에서 침해 원인 공동 1위가 **API·애플리케이션·플러그인 보안 취약점(27%)**과
> **AI 워크로드 관련 클라우드 설정 오류(27%)**다. 우리가 열어둔 구멍이 정확히 그 두 종류다.
> 포트폴리오 관점에서도 "배포 직전에 급히 잠갔다"보다 "설계 단계에서 인증 경계를 정의했다"가 낫다.

작업

1. **`/ws/**` 인증** — SockJS 핸드셰이크에 Authorization 헤더를 실을 수 없으므로 토큰을
   **STOMP CONNECT 프레임**에 담아 검증한다. `ChannelInterceptor`에서 CONNECT 프레임의
   `Authorization` 네이티브 헤더를 읽어 검증하고, 실패 시 연결을 끊는다.
   - 게이트웨이가 아니라 **모듈(factory·fleet) 쪽**에서 막는다. 게이트웨이는 WS 업그레이드를
     프록시만 하고 프레임을 해석하지 않는다.
2. **M2M(서비스 간) 인증** — `GET /api/factory/layout`이 무인증으로 열려 있는 이유가
   "fleet이 기동 시 읽어야 하는데 토큰 받을 방법이 없어서"다. 서비스 계정 토큰을 도입하고
   이 엔드포인트를 닫는다. 그러면 fleet이 모듈에 직접 붙지 않고 **게이트웨이를 경유**하게 된다.
   - 가장 단순한 형태: 서비스별 고정 클레임(`sub=svc-fleet`, `role=SERVICE`)을 가진 장기 토큰을
     `PLATFORM_JWT_SECRET`으로 서명해 환경변수로 주입. 사용자 토큰과 role로 구분한다.
3. **설정 오류 제거** — `mosquitto.conf`의 `allow_anonymous true` → 계정 인증,
   게이트웨이 CORS `allowedOriginPatterns: "*"` → 실제 오리진 목록. (P15에서 P16으로 이동)
4. **인증 경계 문서화** — `docs/auth-boundaries.md`: 어느 경로가 누구에게 열려 있고
   왜 그런지. 표 하나면 된다. 열어둔 것을 **의식적으로 열었다**는 기록이 방어의 절반이다.

완료 기준

- [x] 토큰 없이 `/ws/factory` STOMP CONNECT 시 연결이 거부된다 — **완료(P16 1파)**
- [x] `GET /api/factory/layout`(실제 경로는 `/api/layout`)에 토큰 없이 접근하면 401이다 —
      **완료(WP2, 2026-09-02)**. `SecurityConfig`의 permitAll 목록에서 제거해
      `anyRequest().authenticated()`에 맡겼다.
- [x] fleet이 서비스 토큰으로 factory 평면도를 받아온다 — **완료(WP2, 2026-09-02)**.
      QMS→factory(`ServiceTokenProvider` + `FactoryQualityClient`), WMS→fleet
      (`ServiceTokenProvider` + `FleetTaskClient`)와 같은 패턴을 fleet에도 신설
      (`com.pixelfleet.location.ServiceTokenProvider`) — `LocationRegistry`가 factory를
      부를 때 `Authorization: Bearer` 헤더를 싣는다. **게이트웨이는 경유하지 않는다** —
      원안은 "게이트웨이 경유"였지만, 이미 있던 QMS/WMS 선례가 프라이빗 네트워크 안에서
      모듈 직접 호출 + 서비스 토큰 방식이라 그 관례를 그대로 따랐다(경유하려면 게이트웨이에
      모듈→모듈 라우트를 새로 뚫어야 하는데, 기존 두 사례가 안 그랬던 이유를 뒤집을 근거가
      없었다). 기동 순서 폴백(하드코딩 노드·엣지)은 그대로 유지 — factory가 늦게 뜨거나
      토큰 검증이 실패해도 fleet은 계속 동작한다.
- [x] 익명 MQTT 접속이 거부된다 — **완료(WP3, 2026-09-02, `docs/pixel-platform-roadmap.md` P15와 겹쳐서 그쪽 계기로 먼저 착수)**. `allow_anonymous false` + password file(컨테이너 기동마다 `MQTT_USERS` 환경변수로 생성, git엔 평문 없음) + ACL(도메인별 topic 접근 범위, wms/qms는 읽기 전용). 6개 서비스 전부 인증 접속 라이브 확인.
- [x] 허용되지 않은 오리진의 브라우저 요청이 CORS에서 막힌다 — **완료(P16 1파)**

주의

- **M2M 도입 시 fleet 기동 순서에 의존이 생긴다.** 게이트웨이가 아직 안 떠 있으면 평면도를
  못 받는다 — 기존 폴백(하드코딩 + WARN)이 그대로 살아 있어야 한다. 폴백을 지우지 말 것.
- **STOMP 인증을 켜면 대시보드가 먼저 깨진다.** 대시보드가 CONNECT에 토큰을 싣도록
  같은 커밋에서 고친다. 서버만 먼저 잠그면 화면이 통째로 죽는다.

1파 완료 기록 (WP0 CORS + WP1 STOMP 인증)

- **범위 분할**: 조사해보니 P16이 게이트웨이+factory+fleet+wms+qms+robot-sim+factory-sim+
  대시보드+infra까지 닿는 넓은 범위였다. 한 번에 승인받기엔 크다고 판단해 WP0(CORS)+WP1
  (STOMP 인증) / WP2(M2M) / WP3(MQTT) 세 파로 나눠 이번엔 앞의 둘만 진행했다.
- **CORS**: 게이트웨이 `globalcors`와 factory·fleet의 `WebSocketConfig`(SockJS 핸드셰이크는
  globalcors를 안 거침) 세 곳 전부 `DASHBOARD_ORIGIN` 환경변수로 통일. 실제로 evil origin은
  게이트웨이·모듈 양쪽에서 403, 정상 오리진은 200 + `Access-Control-Allow-Origin` 확인.
- **STOMP 인증**: `StompAuthChannelInterceptor`를 factory·fleet에 각각 신설(`shared`는
  게이트웨이가 의존할 수 없고 wms/qms는 STOMP가 없어서, `WebSocketConfig`처럼 모듈별로 둠).
  `JwtTokenProvider`를 HTTP 필터와 동일하게 재사용하되 실패 시 조용히 통과가 아니라 CONNECT
  자체를 거부. 대시보드 `usePlatformSocket.ts`에 `connectHeaders` 추가 — 서버 인터셉터와
  같은 계획 실행 단위로 처리해 "대시보드가 먼저 깨지는" 문제를 피했다. `ws-test.html`에
  토큰 없음(거부, "끊김 재연결 중" 지속)/토큰 있음(연결됨) 두 케이스 다 실측 확인.
- **남은 것**: M2M(WP2)과 MQTT(WP3)는 각각 별도 승인 필요 — 설계 초안은 이미 확보돼 있다.
  `docs/auth-boundaries.md`는 WP2·WP3까지 끝난 뒤 마무리 단계로 작성한다.

2파 완료 기록 (WP3 MQTT, 2026-09-02)

- **계기가 P16이 아니라 P15였다**: `docs/pixel-platform-roadmap.md` P15("통합 시나리오 + 배포")
  완료 기준에도 같은 항목("익명 MQTT 접속이 거부된다")이 중복으로 있었고, 그쪽 검증 도중
  (`railway domain` 오조작으로 mosquitto가 잠깐 퍼블릭 노출된 사고) 실제 계기가 생겨 그 자리에서
  WP3를 먼저 끝냈다. P16 문서는 사후에 갱신.
- 구현·검증 상세는 `docs/pixel-platform-roadmap.md` P15 완료 기준 항목과 `docs/deploy-railway.md`
  "반드시 알아둘 점" 3번 참고 — 여기서 중복 기록하지 않는다.
- **남은 것**: M2M(WP2)뿐 — `GET /api/factory/layout` 무인증 문제. 아래 완료 기준 참고.

3파 완료 기록 (WP2 M2M, 2026-09-02 — 이걸로 P16 4개 워크패키지 전부 완료)

- 새 파일 1개(`ServiceTokenProvider`, fleet), 수정 4곳(`LocationRegistry`에 헤더 추가,
  factory `SecurityConfig`에서 permitAll 제거, `LayoutController` Javadoc, 기존 단위
  테스트 2개의 `LocationRegistry` 생성자 호출부 — 새 파라미터를 Mockito 목으로 채움).
- `docs/deploy-railway.md`가 QMS/WMS의 M2M을 "아직 없다/백로그"로 **틀리게** 적어 온 걸
  이번에 같이 정정했다 — 실제로는 P13/P14부터 이미 서비스 토큰을 쓰고 있었다(코드 근거:
  `com.pixelqms.factory.ServiceTokenProvider`/`com.pixelwms.fleet.ServiceTokenProvider`).
  문서가 완료된 기능을 계속 미완료로 적어 둔 사례 — 코드가 앞서가고 문서가 못 따라간
  경우였다.
- 검증: control-service `./gradlew test` 전체 통과(영향받은 `LaneGraphTest`/
  `GraphCostAwareAssignmentPolicyTest` 포함), oee-service `./gradlew test` 28/29 통과
  (나머지 1개는 이 환경 Testcontainers/Docker 이슈로 기존부터 실패 — 무관).

---

### P17. MCP 서버 (`platform/mcp-server`, :9005)

> **AI 에이전트가 이 공장을 조회할 수 있게 만든다.** 2026-07-30 착수한 과기정통부·IITP
> '실세계 능동행동형 에이전틱 AI 기술개발'(26개 산학연·180억) 세부 과제에 **MCP 표준 기반
> 멀티에이전트 협업**과 **기업 ERP를 스스로 이해하는 업무 동료**가 들어갔다.
> 컴포저블 아키텍처의 자연스러운 다음 소비자가 사람이 아니라 에이전트라는 뜻이다.

작업

1. `platform/mcp-server/` — 게이트웨이(9000) 뒤가 아니라 **옆에 서는 별도 프로세스**.
   게이트웨이의 공개 REST를 MCP 도구로 감싼다. 모듈 DB에 직접 붙지 않는다(원칙 2).
2. 초기 도구는 **읽기 전용 4개**로 좁게 시작한다.

   | 도구 | 내부 호출 |
   |---|---|
   | `get_equipment_status` | `GET /api/factory/equipments` |
   | `get_oee` | `GET /api/factory/oee?equipmentId=&from=&to=` |
   | `get_fleet_state` | `GET /api/fleet/robots` |
   | `find_events` | `GET /api/factory/events?type=&from=&to=` |

3. **인증** — MCP 서버가 서비스 토큰(P16)으로 게이트웨이를 경유한다. P16 선행 필수.
4. 쓰기 도구(작업지시 착수, 운송 작업 생성)는 **1차 범위 밖.** 읽기가 안정된 뒤 별도 승인.

완료 기준

- [ ] MCP 클라이언트에서 도구 4개가 보이고 실제 데이터가 반환된다
- [ ] "CNC-01 오늘 OEE 얼마야?" 같은 질의가 도구 호출로 해소된다
- [ ] MCP 서버를 내려도 대시보드·모듈은 정상 동작한다 (컴포저블 검증)
- [ ] 토큰 없이 뜬 MCP 서버는 도구 호출 시 401을 그대로 노출한다 (조용히 실패 금지)

주의

- **쓰기 도구를 성급히 열지 않는다.** 에이전트가 작업지시를 착수시킬 수 있게 되는 순간
  "저장 연타 = TOCTOU" 문제가 사람보다 빠른 속도로 재현된다. 3층 방어가 먼저다.
- **언어 = Python(FastMCP)로 결정.** 도구 스펙이 얇아 가장 빠르고, Python 학습 목적에도 맞다.
  Java 통일(플랫폼 스택 일관성)은 검토했으나 이 조각에는 과하다고 봤다.
  → 플랫폼의 Gradle 빌드 흐름 밖에 있으므로 `platform/mcp-server/`는 **자체 의존성 관리
  (`pyproject.toml` 또는 `requirements.txt`)**를 갖고, `scripts/`에 기동 스크립트를 따로 둔다.

---

### P18. 추적 그래프 — LOT/파렛트 4단 트리 (온톨로지 방향)

> 이미 `docs/pixel-platform-roadmap.md` 0-A에 "스캔 한 번으로 전 계층을 잇는 화면이 이
> 도메인의 하이라이트"라고 적어둔 것과 **같은 항목**이다. 여기서는 방향의 근거만 덧붙인다.
>
> 전자신문 ET단상(위즈코어 박승훈 대표) — 제조 AX의 병목은 모델 성능이 아니라
> **설계 데이터와 제조 데이터가 다른 언어로 존재해 연결되지 않는 것**이고, 그걸 잇는
> 게 온톨로지다. 조선일보 피지컬 AI 투자 기사 — **"데이터 순환 고리를 움켜쥔 회사에
> 프리미엄이 집중"**. 두 기사가 같은 말을 한다.

선행: P13(WMS·품목 마스터) + P14(QMS·부적합)가 있어야 이을 것이 생긴다. **P16·P17보다 뒤.**

방향만 기록

- 작업지시번호 / LOT / 작업LOT / 생산LOT / 자재박스바코드 / **파렛트번호** 중
  아무거나 입력 → 우선순위 `UNION ALL`로 해소 → 4단 트리
- 여기까지는 관계형으로 충분하다. **그래프 DB를 성급히 도입하지 않는다.**
  트리 깊이가 고정(4단)이고 순환이 없으면 재귀 CTE로 끝난다.
- 그래프가 필요해지는 지점은 "불량 → 원인 공정 → 자재 → 설계 특성"처럼
  **경로가 가변이고 역방향 탐색이 주가 될 때**다. 그때 판단한다.

---

### P19. Fleet API를 M4 모양으로 — 현장 규격을 기본기로 삼는다 ✅ 완료 (2026-08-06, 일부 후속 항목 제외)

> 실 운영 중인 한 현장은 WMS가 **M4**(상용 AMR 군집관제 서버)를 거쳐 실물 로봇을 부린다.
> 근거 문서: 해당 현장에서 확보한 M4 Fleet API 명세서(HTTP + WebSocket, 176쪽).
>
> **목적은 호환 레이어가 아니다.** 남의 클라이언트를 받으려고 M4를 흉내내는 게 아니라,
> 현장에서 검증된 API 모양새를 **pixel-fleet 자체 API의 기본형**으로 삼는다.
> 실물 관제 서버가 왜 그 개념들을 갖는지(스텝, 봉인, 씬, 요청-응답 봉투)를
> 내 손으로 구현하면서 체득하는 것이 목표다. 그 결과로 자연히 M4를 쓰는 현장
> 통합 코드와도 대화가 통하게 되면 덤이다.

M4에서 가져올 뼈대 (fleet API를 이 모양으로 개편)

- **주문 = 다단 스텝.** `orders/create`가 steps 리스트(location + 동작)를 받고,
  미봉인 주문에 `add/update/delete-steps`, `stepFixed`(봉인) 개념까지.
  transport_tasks의 출발→도착 단일 이송을 스텝 리스트로 일반화한다 — 내부적으로
  이미 2구간(픽업+하역, 엘리베이터 인수인계)을 쓰고 있어 토대는 있다.
  **이게 P19의 몸통이다.** 지금의 handoff 특수 처리가 "스텝 2개짜리 주문"으로 녹는다.
- **주문 생애주기 동사를 넓힌다**: cancel(-batch)/suspend/unsuspend/
  complete-order(수동 완결)/retry-failed. 지금은 재시도만 자동이고 운영 개입 동사가 없다.
- **externalId 관례**: 상류(WMS)가 자기 번호를 실어 보내고 완료 통지에서 그걸로 되찾는
  구조 — 이미 taskCode로 하고 있으니 이름과 자리만 규격에 맞춘다.
- **WS 요청-응답 봉투**: `id/action/content` → `action::Reply` + `replyToId`.
  지금의 단방향 push(STOMP broadcast)에 "질의"가 없다 — 위치만 골라 묻는
  `RobotsPositionOnly::Query` 같은 걸 이 봉투로 연다.
- **로봇 운영 동사**: off-duty(배차 제외)/disabled/clear-alarm — 지금은 로봇을
  운영에서 뺄 방법이 시뮬레이터를 끄는 것뿐이다.

주의

- **176쪽 전량을 좇지 않는다.** DI/DO, soft-EMC, rbk-request, 지도 리소스 잠금 등
  하드웨어 제어 계열은 robot-sim에 대응물이 없다 — 개념만 문서로 남기고 뺀다.
- scene은 당장 1개(우리 공장)다. 다중 scene 구조를 미리 파지 말고,
  API 시그니처에 sceneId 자리만 잡아 둔다.
- 스텝 일반화는 fleet 도메인 개편이라 **마이그레이션·배차·교통통제가 다 닿는다.**
  착수 전에 계획을 먼저 세우고 승인 받는다(작업 규칙).

완료 기록 (P19-1은 이미 완료돼 있었고, 이번에 나머지를 마쳤다)

- **주문 생애주기 동사**: cancel(-batch)/suspend/unsuspend/complete-order/retry-failed —
  전부 `OrderController`(`/api/orders`)로 신설. cancel은 `OrderStatus`에
  `EXECUTING→CANCELLED`를 추가해 정직한 전이로 만들었다(faultOut 우회 아님).
  retry-failed는 `resetForRetry`를 그대로 재사용하지 않고 `fault`까지 지우는
  `retryAfterFault` 래퍼를 얹었다(안 그러면 영원히 안 배차되는 좀비 주문이 남는다).
- **externalId 관례**: `TaskController.create()`가 이제 `OrderCodeGenerator`(V9 시퀀스)로
  자체 `orderCode`(`FO-########`)를 발급하고, WMS의 taskCode는 externalId로만 들어간다.
  WMS 쪽 코드 변경 0건(왕복 확인 완료).
- **로봇 운영 동사**: off-duty/disabled/clear-alarm — `robots` 테이블(V8)에 조작자 전용
  플래그로 추가(텔레메트리로 안 바뀌게 Postgres 마스터에 둠). `findAvailable()`에서 걸러
  배차 후보에서 빠진다.
- **WS 요청-응답 봉투**: `/app/query` + `/topic/query-reply`, 첫 질의
  `RobotsPositionOnly::Query`. `ws-test.html`(깨져 있던 `/ws` 경로도 `/ws/fleet`로 고침)에서
  실측 확인.
- 로컬 스택(`-Stack e2e`)에서 curl/SQL fixture/브라우저로 전부 실동작 확인 — 그 과정에서
  `cancel()`이 미배차(로봇 없음) 주문을 취소할 때 `TrafficController.release(null)`이
  `ConcurrentHashMap` null 키로 NPE 나는 실버그를 하나 잡아 고쳤다.

의도적으로 미룬 것 (범위 밖, 별도 승인 필요)

- **cancel의 물리적 중단** — 지금은 서버만 손을 뗀다. 이동 중이던 로봇에게 실제로 멈추라고
  MQTT `CANCEL` 커맨드를 보내는 건 robot-sim 모듈까지 건드려야 해서 이번 계획(control-service +
  dashboard)의 선언된 범위 밖으로 남겨뒀다. robot-sim의 `VirtualRobot.clearOrder()`가 이미
  "중단"을 언급해둬서 토대는 있다.
- **M4형 `orders/create`**(steps 배열 입력, add/update/delete-steps) — 생성은 계속
  `TaskController`(호환 어댑터) 전담. `OrderController`는 조작자 동사만 다룬다.
- **WS 세션 타겟 응답** — 지금은 `/topic/query-reply` 브로드캐스트 + `replyToId` 클라이언트
  필터링. P16(STOMP 인증)이 진짜 `Principal`을 주면 세션 타겟으로 올리는 걸 검토.

---

### P20. 멀티 플랜룸(Building 컴포저블) + 동적 장애물 · 레이아웃 버전 관리 ✅ 완료 (2026-08-06)

> 세부 설계·결정 근거·검증 이력: [`docs/p20-layout-routing-design.md`](./p20-layout-routing-design.md)
> — P20-1(그래프 데이터 모델) ~ P20-5(장애물 반영 배차)까지 전부 완료.

> 지금 평면도는 건물 3개(창고동/생산동/품질동)가 **하나의 68×26 그리드 위에 배치만 나뉜** 구조다.
> `LaneGraph`의 경로 계산은 건물을 모른다 — 통로 y(9/18)와 커넥터 x 8개가 **컴파일 타임 상수**로
> 박힌 4단 고정 규칙(수직→통로→수평→수직)이라, 그래프 탐색이 아니다. 층도 창고동에만 있고 그마저
> 같은 x,y를 층마다 복제한 형태다(로봇은 층을 안 넘음). 장애물 개념 자체가 없다 — 지금의 "구간
> 점유"는 로봇끼리 충돌을 피하는 트래픽 컨트롤이지, "이 구간이 막혔으니 우회"가 아니다.
>
> Building-A(가공동)/Building-B(조립·물류동)처럼 독립된 레이아웃 레이어를 인터페이스로 조립하고,
> 설비 추가·통로 봉쇄 같은 변화를 시뮬레이터가 동적으로 반영하게 만들면, "정적 룰 라우터 →
> 동적 그래프 라우터" 전환 자체가 기술 포트폴리오로 보여줄 게 많은 작업이 된다.

목표

- 건물(Building)을 **독립된 로컬 좌표계 + 로컬 레인그래프**로 만들고, 건물 사이는 명시적
  "게이트 노드"(엘리베이터/도어)로만 연결한다 — 모듈이 계약으로만 통신하는 원칙을 레이아웃에도 적용.
- `LaneGraph`의 고정 4단 규칙을 **노드-엣지 그래프 + A*/Dijkstra 경로탐색**으로 교체해서,
  엣지 비용을 동적으로 바꿀 수 있게 한다(그래야 "우회"가 성립한다).
- 장애물 발생/해제를 **이벤트**로 다룬다 — 이벤트가 단일 진실 공급원이라는 원칙 그대로.
- 레이아웃 자체에 버전을 매겨서, "이 시점 이후 평면도가 바뀌었다"를 추적 가능하게 한다.

작업 (제안 — 착수 전 세부 설계 확정 필요)

1. `layout_buildings`에 로컬 좌표계 개념을 추가하고, 건물별 `layout_edges`(노드 A↔B, 기본 비용,
   `blocked_from`/`blocked_until`) 테이블을 신설한다. 지금의 `CONNECTOR_X`/통로 y는 이 테이블의
   초기 데이터로 흡수한다(기존 3개 건물 좌표는 그대로 유지 — 로봇 시야에선 하위호환).
2. `LaneGraph.plan()`을 그래프 탐색으로 교체한다. 건물 간 이동은 게이트 노드(엘리베이터 등)를
   경유하는 엣지로 표현 — Building-A/B 추가는 새 노드·엣지 삽입만으로 가능해야 한다(코드 변경 없이).
3. 좌표 4곳 사본(`LaneGraph` / `LocationRegistry.FALLBACK_NODES` / `DemoTaskGenerator.FLOWS` /
   robot-sim `NodeMap`) 동기화 방식을 다시 설계한다 — DB가 정본이 되면 나머지는 캐시+폴백으로
   좁혀야 한다(`NodeMapLayoutConsistencyTest`가 지금 하는 역할은 유지하되 대조 대상을 바꾼다).
4. 장애물 이벤트를 신설한다 — 시뮬레이터가 MQTT로 `OBSTACLE_ADDED`/`OBSTACLE_CLEARED`(대상: 엣지
   ID, 사유, 유효기간)를 발행하면 fleet이 구독해 해당 엣지 비용을 무한대로 바꾸고 진행 중인 경로를
   재계산한다. 대시보드는 막힌 구간을 지도에 표시한다.
5. 레이아웃 버전 — `layout_version` + 유효기간 컬럼(또는 별도 테이블)으로 "이 평면도가 언제부터
   유효한지"를 기록한다. 과거 이벤트를 다시 볼 때 당시 평면도로 렌더링할 수 있어야 한다(옵션
   범위는 설계 단계에서 정한다).

완료 기준

- [x] Building-A/B 두 개를 게이트 노드로 연결한 경로가 실제로 계산되고, 로봇이 건물을 넘어
      이동한다 — 코드 확인: `LaneGraph`가 `PriorityQueue` 기반 Dijkstra로 교체됨(P20-2),
      `LaneGraphTest`에 건물 간 경로 비교 테스트(`다른_건물_노드간_경로는_...`) 존재
- [x] 시뮬레이터가 임의 구간을 막으면(이벤트 발행) 진행 중인 로봇이 그 구간을 피해 재경로한다 —
      **실측**: `dev-up.ps1 -Stack full -Demo`로 띄운 살아있는 데모의 통합 이벤트 타임라인에
      `[A] LAYOUT_OBSTACLE_CLEARED — 엣지 JCT-4-L~JCT-4-U 해제 (정리 완료)`가 실제로 찍힘
      (장애물이 발행→해제까지 도는 걸 재현 없이 실행 중 자연 발생으로 관측). 코드 근거는
      `ObstacleService`/`ObstacleStore` + `LaneGraphTest`의
      `엣지가_막히면_그_엣지를_안_쓰고_다른_길로_우회한다` 테스트
- [x] 기존 3개 건물(WH/PROD/QC) 데모 시나리오가 회귀 없이 그대로 동작한다(하위호환) —
      코드 확인: `LaneGraphTest`의 `다른_건물_노드간_경로는_옛_LaneGraph_계산값과_비용이_일치한다`가
      이 회귀를 직접 검증한다
- [x] 좌표 정합 검증(현재 `NodeMapLayoutConsistencyTest`의 후속)이 새 정본(DB) 기준으로
      계속 불일치를 잡아낸다 — 코드 확인: `NodeMapLayoutConsistencyTest`가 마이그레이션에서
      파싱한 노드·폭·높이·상층 통로 y를 대조한다

> 코드·테스트 근거로 확인한 것이며, 실제 대시보드에서 로봇이 우회하는 것을 눈으로 본
> "실측"은 아직 없다.

주의

- **레인그래프 전면 교체는 배차·교통통제·robot-sim까지 다 닿는다.** P19의 스텝 일반화보다 반경이
  넓다 — 세부 설계(그래프 표현 방식, 엣지 비용 모델, 게이트 노드 프로토콜)를 먼저 문서로 확정하고
  승인받은 뒤 착수한다(작업 규칙).
- **동적 장애물이 배차 정책(배터리 인지 최근접 배차)과 상호작용한다** — 막힌 구간 때문에 "가장
  가까운" 로봇이 실제로는 더 먼 우회로를 타야 할 수 있어, 배차 시점의 거리 계산도 그래프 비용
  기반으로 다시 봐야 한다.
- 지금 단계에서는 **자유 형태 장애물 회피(연속 좌표계 상의 임의 다각형 회피)까지는 가지 않는다** —
  레인그래프의 엣지 단위 차단으로 범위를 좁힌다. 그 이상은 별도 항목으로 분리한다.

---

### P21. 창고동 렉 취출 로봇(랙 피더) — WMS 물리 흐름 정합 ✅ 완료 (2026-08-13, 옵션 A)

> 세부 설계·결정 근거(D1~D10)·실행 단계: [`docs/p21-warehouse-rack-feeder-design.md`](./p21-warehouse-rack-feeder-design.md)

> 코드로 확인한 지금의 구멍: factory `layout_racks`(렉 27기, 실좌표·열·단·용량)와 WMS
> `locations`(렉=로케이션, 실재고)가 이미 있는데, WMS V5 마이그레이션이 렉의 `node_code`를
> 렉의 실제 위치가 아니라 그 층 피킹존 노드로 덮어써 뒀다("위층 물건은 리프트로 내려온 셈
> 친다"는 주석 그대로). 그래서 지금 지도의 렉 27개는 적재율 색만 바뀌는 배경 그림이고,
> 어떤 AMR도 렉 근처에 실제로 간 적이 없다 — "이미 거기 물건이 있다"고 치는 것이다.

목표

- AMR과는 다른 로봇 종류 **랙 피더(Rack Feeder)** 를 도입한다 — 창고동 안, 자기 피킹존
  밖으로는 나가지 않는다(AMR의 `LaneGraph` 공유 레인망에 올라가지 않는다).
- 출고지시가 만드는 운송이 "렉 → 피킹존"(랙 피더) → "피킹존 → 최종 목적지"(AMR, 기존
  엘리베이터 handoff와 같은 자리에서 두 주문으로 이어진다)의 **실제 두 구간**이 되게 한다.
- WMS 쪽 코드는 무변경 — 렉이 어느 피킹존과 가까운지는 fleet이 좌표로 계산한다(지금 WMS가
  떠안고 있는 계층 위반도 함께 정리).

핵심 결정(design doc D5): 지금의 층간 handoff(`crossFloor` 판정, `handoff_destination`/
`handoff_of`/`available_at`)를 "로봇 풀이 바뀌는 경계"로 일반화해서 재사용한다 — 새 상태
기계를 만들지 않는다. 이 리팩터링이 이번 작업에서 가장 조심해야 할 지점(기존 엘리베이터
handoff 회귀 위험)이라 design doc 6절에 롤백 수단을 별도로 적어 뒀다.

완료 기준 — 전부 실기동 확인(근거: design doc 5·7절)

- [x] 렉 재고로 낸 출고지시가 랙 피더의 실제 이동(렉 도착→취출 대기→피킹존 이동)을 거친 뒤
      AMR이 이어받아 최종 목적지까지 옮긴다
- [x] 전 과정이 `fleet_events`에서 한 흐름(handoff_of로 연결된 두 주문)으로 추적된다
- [x] 기존 AMR 전용 흐름·엘리베이터 층간 handoff가 회귀 없이 그대로 동작한다
- [x] WMS 코드는 한 줄도 안 바뀐다(로케이션 마이그레이션 데이터만 변경)

주의

- **`LaneGraph`/`TrafficController`에는 의도적으로 손대지 않는다**(design doc D2) — P20에서
  가장 검증에 공들인 부분이라, 랙 피더의 이동은 로컬 직선 이동으로 완전히 분리해 그 위험을
  피한다.
- **존당 로봇 1대로 시작하면 2026-08-04에 실제로 겪은 "자원 풀을 쪼갠 뒤 여유 0" 사각지대를
  그대로 물려받는다.** 설계로 완전히 없앨 수 없어 운영 관찰 후 조정하는 단계로 남겨 뒀다
  (design doc D10).
- 착수 전 design doc 9절의 열린 질문(로봇 이름·대수·취출 소요시간·D5 범위) 확인 필요.

---

### P22. AMR/AGV 경계 확정 — 창고동 1층 AMR 진입 차단 ✅ 완료 (2026-08-25, 배포 검증까지)

> 세부 설계·결정 근거(D1~D6)·실행 단계: [`docs/p22-amr-agv-boundary-design.md`](./p22-amr-agv-boundary-design.md)

> 요청 원문: "AMR이 창고동 까지는 들어오지 않는 구조", "창고동에서는 AMR모다는 AGV가
> 움직여야할꺼같아". P21은 "렉 → 피킹존" 구간만 랙 피더에게 맡겼다 — 창고동 1층의
> 나머지(입고장·출하장·도크·엘리베이터 승강장)는 여전히 AMR이 `LaneGraph`를 타고 자유롭게
> 드나들었다. 이번 작업은 그 경계를 창고동 **건물 전체**로 넓힌다.

목표

- AMR은 창고동(WH) 1층 안쪽 어디에도 배차되지 않는다 — 렉뿐 아니라 입고장·출하장·도크·
  엘리베이터 승강장까지 전부.
- P21의 "랙 피더"를 **AGV**로 이름을 일반화하고(`RobotType.RACK_FEEDER` → `AGV`), 담당
  범위를 렉에서 창고동 1층 전체로 넓힌다 — 2·3층은 P21 그대로(좁은 피킹존 단위) 유지.
- AMR↔AGV 경계를 **게이트 노드**(`WH-GATE-U`/`WH-GATE-L`)로 물리적으로 고정한다 — WH·PROD
  건물 사이 기존 직통 연결로를 게이트에서 한 번 끊는다(라우팅 비용은 분할 전후 동일하게
  보존).

핵심 결정(design doc D3): 1층은 기존 `WH-PICK` 존 코드를 그대로 재사용해 "창고동 1층
전체가 하나의 넓은 AGV 존"으로 취급하되, 2·3층은 P21의 층 경계 제약(자기 존 밖 엘리베이터로
못 감)을 그대로 물려받는다 — 1층만 엘리베이터 접근을 예외로 허용하는 층 기반 분기가 이번
작업에서 가장 조심해야 할 지점이다(design doc 6절 리스크 참고).

완료 기준 — 로컬 구현·검증에 이어 Railway 배포 검증까지 전부 완료(근거: design doc 5·7절)

- [x] factory V17/fleet V11 마이그레이션 작성(게이트·PROD-DOCK 추가, RACK_FEEDER→AGV 변환)
- [x] `OrderService`(`requiredPool`/`handoffNodeFor`/`createHandoffOrder`) 확장
- [x] robot-sim(`NodeMap`/`Simulator`/`ObstacleSimulator`/`VirtualRobot`) 반영
- [x] 대시보드(게이트 마커, AGV 이름 변경, 노드 필터) 반영
- [x] 로컬 Gradle/TS 빌드 검증 — control-service/robot-sim 테스트 전부 통과(검증 중 게이트
      분할을 반영 못 한 `LaneGraphTest` 2건 발견·수정), `tsc --noEmit`/`vite build` 통과
- [x] **Railway 배포 후 실기동 확인**(2026-08-25) — V17/V11 Flyway 적용 로그 확인,
      `POST /api/orders`로 AMR 게이트 정차·1층 AGV 엘리베이터 예외 허용·2·3층 AGV 층
      경계 거부 3가지 전부 직접 검증, 대시보드 라이브 이벤트 타임라인에서 실제 게이트
      핸드오프 트래픽 확인. 세부: design doc 5절 P22-5

부수 발견 및 수정(배포 검증 중)

- `pixel-fleet`에 `LAYOUT_URL` 환경변수가 아예 없어서(배포 가이드에도 누락) factory의
  실제 레이아웃을 받아본 적이 없고 계속 하드코딩 폴백(V16 시절 좌표)으로만 동작 중이었다
  — 코드상 폴백 상수가 V17 이후 값과 동기화돼 있어 겉보기 동작은 맞았지만, factory와의
  실시간 동기화 자체가 조용히 끊겨 있었다. `LAYOUT_URL` 추가로 즉시 해결, 재시작 로그로
  실제 레이아웃 54노드/54엣지 로드 확인.

주의

- **게이트 비용 분할이 라우팅 결과를 바꾸지 않는지가 핵심 검증 포인트다** — 기존 직통 간선
  (`JCT-14↔JCT-27`, cost 19)을 게이트를 낀 두 구간(13+6=19)으로 정확히 등가 분할했지만,
  `NodeMapLayoutConsistencyTest`로 좌표·비용 일치를 반드시 확인해야 한다.
- **`LaneGraph`/`TrafficController`에는 이번에도 손대지 않는다**(P21 D2를 그대로 계승) — 게이트도
  평범한 그래프 노드일 뿐, AGV는 여전히 로컬 직선 이동이라 그래프에 올라가지 않는다.

---

### P23. 파렛트 단위 재고 — WMS를 로봇 규격에 맞춘다 ✅ 완료 (2026-08-25)

> 세부 설계·결정 근거(D1~D9)·열린 질문: [`docs/p23-pallet-unit-design.md`](./p23-pallet-unit-design.md)

> 외부 자료(`amr 사양요구서.docx` — IPLUS MOBOT EMMA 600K 기술협의서, Cloudia/M4 Fleet
> API 문서, 동방플라스틱 WMS 실무 스키마) 분석에서 나온 항목. EMMA 600K는 품목이 아니라
> **1100×1100 팔레트를 잭업해서** 옮기는데, 지금 `stocks(location_id, item_id, quantity)`는
> 로케이션 안의 품목 수량만 뭉텅이로 갖고 있어 "몇 장의 파렛트로 나뉘어 있는지"가 없다.
> P21/P22가 "로봇이 어디까지 갈 수 있는가"(AMR/AGV 경계)는 이미 정교하게 잡아 뒀지만,
> "로봇이 무엇을 들 수 있는가"는 전혀 다루지 않았다 — 그 축을 채우는 첫 착수 지점이다.

목표

- 로케이션이 재고를 직접 갖던 것을 **로케이션 → 파렛트 → 재고**의 3단으로 바꾼다 —
  `pallets`(QR 코드·위치·상태) 신설, `stocks`를 (로케이션,품목)에서 (파렛트,품목)으로 재정의.
- 출고를 **파렛트 단위 전량 이동**으로 바꾼다(EMMA 600K는 부분 피킹을 못 한다 — 리프팅식
  로봇이라 파렛트를 통째로만 옮긴다).
- 사양서 실측치(파렛트 총중량 500kg 미만, 로케이션의 파렛트 슬롯 용량)를 입고 검증에 반영한다.
- **factory/fleet DB·API는 원칙적으로 무변경**(전제, design doc D8) — fleet 계약에
  파렛트 코드를 실을지(D6)는 열린 질문으로 남겨 별도 승인 대상.

핵심 결정(design doc D2): `stocks.location_id`를 제거하고 파렛트에서만 위치를 갖는다 —
되돌리기 어려운 변경이라 nullable 컬럼 추가 → 백필 검증 → not null 전환 → 구컬럼 제거의
2단계 마이그레이션으로 진행했다(design doc 6절 리스크·롤백 참고). 9절 열린 질문 4가지는
전부 권장안대로 확정: 1:1 제약 잠금(D2), fleet 계약 확장(D6) 동시 실행, 중량 검증(D4)
포함, 기존 출고 API 병행 어댑터(P19 패턴 재사용).

완료 기준 — 전부 실기동으로 확인(2026-08-25, factory+fleet+wms 풀스택 로컬 구동)

- [x] 입고지시가 파렛트를 만들고 QR 코드(`PLT-########`)로 조회된다 — 중량 초과(500kg↑)·
      슬롯 초과 입고는 거절됨을 실기동 확인
- [x] 출고지시가 파렛트를 통째로 대상으로 하고, 부분 잔량이 남지 않는다 — 수량 불일치 시
      거절, FIFO 자동 선택·명시적 `palletCode` 지정 둘 다 확인
- [x] 500kg 초과 입고, 슬롯 초과 입고가 거절된다
- [x] 운송 완료 MQTT 통지 → 파렛트 RETIRED + 재고 삭제, 중복 통지 멱등성 확인
- [x] fleet `fleet_orders.material_id`에 파렛트 코드가 실려 조회됨을 확인(D6)
- [x] `V7` 마이그레이션을 실제 Postgres에 적용해 `ddl-auto: validate` 통과, 27개 로케이션
      백필 결과(파렛트 23장, qty=0 로케이션 5곳 제외) 직접 확인
- [x] 대시보드 렉 적재율 표시 무변경(D8 전제) — 집계 쿼리 구조상 로케이션 합산이라 영향 없음

주의

- 외부 근거 자료(`D:\happyeon\05.DongBang\Book`)는 실제 고객사 산출물이라 물리 규격
  수치와 스키마 구조만 참고했다 — 품번·거래처·주문 등 실데이터는 가져오지 않았다.
- e2e 검증 중 로컬 `pixelfleet` DB가 예전 세션의 마이그레이션 드리프트(V5 체크섬 불일치)로
  기동에 실패해 볼륨을 리셋했다 — P23 변경과 무관한 로컬 환경 문제였다.

---

### P24. WMS→fleet을 M4형 다단 스텝 `orders/create`로 전환 ✅ 완료 (2026-08-25)

> 세부 설계·결정 근거(D1~D4): [`docs/p24-m4-order-creation-design.md`](./p24-m4-order-creation-design.md)

> 원래 P24로 잡았던 "`OrderStep.action`+멱등키"는 P19가 이미 끝내 놨다는 게 P23 설계 중
> 드러났다(`forLoad`/`forUnload`·`externalId`·`stepFixed` 전부 기존). 대신 P19 스스로
> "의도적으로 미룬 것"에 적어 둔 **M4형 `orders/create`(스텝 배열 입력)** 이 실제로
> 비어 있었다 — `OrderController`(`/api/orders`)에 조작자 동사만 있고 생성 엔드포인트가
> 없어서, WMS도 여전히 구식 2필드 `TaskController`(`/api/tasks`) 호환 어댑터를 거쳤다.

목표

- fleet에 `POST /api/orders`(스텝 배열, M4형) 신설 — 기존 `OrderService.create(...)`
  (P19가 이미 완성)을 그대로 호출하는 얇은 진입점.
- WMS `FleetTaskClient`가 이 신규 엔드포인트로 갈아탄다 — 픽업/하역 2스텝을 직접 조립해
  보낸다. 우선순위 문자열→정수 변환을 fleet 관례(0~3)로 통일.
- **`TaskController`(`/api/tasks`)는 지우지 않는다** — 대시보드 `TaskPanel.tsx`가 아직
  이 경로로 수동 작업을 만든다(확인 완료). 소비자가 전부 옮겨간 뒤에야 삭제 대상.

완료 기준 — 전부 실기동으로 확인(2026-08-25)

- [x] WMS 출고지시 → fleet `POST /api/orders` 호출 → `GET /api/orders`에서 스텝 2개
      (forLoad/forUnload)·`externalId`·`materialId`(파렛트 코드) 확인
- [x] MQTT 완료 통지 → 재고 차감까지 새 경로로도 P23과 동일하게 동작
- [x] 기존 `POST /api/tasks`(대시보드 경로) 무변경 동작 확인 — `materialId: null`로 응답,
      회귀 없음
- [x] 양쪽 모듈 컴파일·fleet 전체 테스트(`./gradlew test`) 회귀 없음

주의

- `OrderResponse`/`TaskResponse` 둘 다 `materialId`를 노출하도록 P23~P24에 걸쳐 맞췄다.
- `add/update/delete-steps`, fleet 레벨 `ref_uuid` 충돌 검사는 범위 밖으로 남겼다(design
  doc 5절) — 지금 유일한 실사용 호출부(WMS)가 필요로 하지 않는다.

---

### P25. 로봇 규격을 라우팅에 강제한다 — 통로폭 + 정격 적재량 ✅ 완료 (2026-08-25)

> 세부 설계·결정 근거(D1~D7): [`docs/p25-robot-spec-routing-design.md`](./p25-robot-spec-routing-design.md)

> P21/P22가 일관되게 "`LaneGraph`/`TrafficController`에는 손대지 않는다"를 반복해 온
> 부분을 이번에 처음 건드렸다 — 사용자가 명시적으로 "데이터만 들고 대시보드 표시"가
> 아니라 "라우팅에 실제 강제"를 선택했다. 대신 최소 침습으로 한다: 새 시그니처는 전부
> 하위호환 오버로드로 추가하고, 막는 조건 하나만 기존 장애물 판정 옆에 나란히 둔다.

목표

- factory `layout_edges.width_mm`(통로 물리 폭) 신설, 전부 2000mm로 시드(실측 없음 —
  두 임계값을 넉넉히 웃도는 안전한 근사, 좁은 구간을 인위로 만들지 않는다).
- fleet `LaneGraph.plan()`에 `loaded` 오버로드를 추가하고, 다익스트라 완화 단계에서
  로딩 상태별 최소폭(사양서 §4.3 — 공차 950mm/적재 1400mm) 미달 엣지를 장애물과 같은
  방식으로 건너뛴다. `TrafficController`가 구간을 배타적으로 잠그므로 "단일 통로" 값만
  쓴다(양방향 값은 이 구조에서 의미가 없다).
- 로봇 규격은 로봇별 필드가 아니라 정책 상수로 둔다(이 포트폴리오의 AMR은 전부 EMMA
  600K 한 모델 — `MIN_BATTERY_PERCENT`와 같은 패턴).
- 정격 적재량(600kg) 초과 화물은 fleet `POST /api/orders`(P24) 생성 시점에 거부한다
  (`weightKg` 선택 필드, D7) — 배차 단계 필터가 아니라 생성 시점 거부로 충분하다.

핵심 결정(design doc 1절): `TrafficController`가 구간을 배타적으로 잠그므로 "양방향
동시 통행"이 애초에 없다 — 임계값이 2가지(공차/적재)만 있으면 되고, 로딩 상태는 이미
`FleetOrder.loaded`(P19)로 추적되고 있어 새 상태를 만들 필요가 없었다.

완료 기준 — 전부 실기동으로 확인(2026-08-25, factory+fleet+robot-sim 풀스택)

- [x] `GET /api/layout`에서 모든 엣지가 `widthMm: 2000` 노출
- [x] 700kg 주문 생성 거부(`POST /api/orders`), 500kg는 정상 생성·라우팅
- [x] 실제 배차·라우팅이 `loaded=false`/`loaded=true` 양쪽 다 실기동으로 실행됨을 확인
      (로봇 8대 온라인, 여러 주문이 DONE까지 완주, 예외 0건)
- [x] 신규 단위 테스트(폭 판정 함수 경계값, 폴백 그래프에서 `loaded` 무관 결과 동일)
      + 기존 `LaneGraphTest`(6건)·`OrderServiceRegressionTest`(모킹 스텁 3인자로 갱신)·
      `GraphCostAwareAssignmentPolicyTest` 전부 회귀 없음

주의

- **이번 배포에서 폭 강제 로직은 "한 번도 실제로 막지 않는다"** — 2000mm 시드가
  950/1400mm를 항상 웃돌기 때문이다(의도된 결과, design doc 6절). 메커니즘 자체의
  정확성은 단위 테스트(경계값 1200/1400/950mm)로 증명했다.
- WMS가 파렛트 무게(`Item.unitWeightKg`, P23)를 fleet에 실어 보내는 배선은 범위 밖으로
  남겼다 — fleet 쪽 검증 자리(`weightKg`)만 만들었다.

---

### P26. 안전재고 기반 보충 작업 자동 생성 ✅ 완료 (2026-08-25)

> 세부 설계·결정 근거(D1~D8): [`docs/p26-safety-stock-replenishment-design.md`](./p26-safety-stock-replenishment-design.md)

> 원래 "난이도 낮음"으로 분류했으나, 실제로 뭘 옮기는지 따라가 보니 새 주문 종류(내부
> 이동)가 필요하다는 게 드러났다 — 대신 P23~P25가 만들어 둔 조각(파렛트 FIFO 선택,
> M4형 `orders/create`, `materialId`)을 전부 재사용해서 실제로는 작았다. 원본 자료
> (상품리스트.xlsx)의 안전재고 컬럼은 품목 단위지만, 로봇이 실제로 일을 만들어 내는 건
> "이 로케이션은 낮은데 저 로케이션엔 아직 있다"는 상황뿐이라 **로케이션 단위**로
> 재해석했다(0절 — 품목 총량 알림은 구매/생산 신호이지 로봇 작업이 아니다).

목표

- `locations.safety_stock_qty` 신규 — 출고 완료로 재고가 줄어든 로케이션이 이 값 아래로
  떨어지면 자동으로 보충 대상이 된다(트리거는 출고 완료 시점 하나뿐 — 주기적 스윕 없음).
- `ReplenishmentOrder` 신설 — `OutboundOrder`와 구조는 비슷하지만 완료 의미가 다르다
  (파렛트를 은퇴시키지 않고 **다른 로케이션으로 그대로 옮긴다**). 도너 파렛트는
  FIFO(P23 D3 재사용), fleet 생성은 M4형 `POST /api/orders`(P24 재사용), 우선순위는
  LOW(고객 출고보다 급하지 않다).
- `StockService.relocatePallet` — 재고 행은 안 건드리고 파렛트의 `location_id`만 옮긴다
  (위치는 파렛트에서만 파생된다는 P23 D2 덕분에 이 한 줄로 끝난다). 도착지 슬롯 검증도
  기존 `assertSlotAvailable`(P23 D7)을 그대로 재사용.

완료 기준 — 전부 실기동으로 확인(2026-08-25, factory+fleet+wms 풀스택)

- [x] 안전재고 지정 로케이션(WH-1F-R08, 임계 50)에서 파렛트 전량 출고 완료 → 보충 지시
      자동 생성(`RPL-00000001`) 확인
- [x] fleet에 M4형으로 생성됨을 확인(steps 2개, `materialId`=도너 파렛트 코드,
      `priority`=0/LOW)
- [x] 완료 통지 후 도너 파렛트(`PLT-LEGACY-000008`, 200개)가 은퇴하지 않고 도착
      로케이션(WH-1F-R08)에서 `LOADED`로 다시 나타남을 DB로 직접 확인
- [x] 중복 완료 통지 재발행해도 두 번 이동하지 않음(멱등성) — 예외 0건

주의

- 이번 시연에서 도미노(도너 자신도 안전재고 미달이 되는 연쇄)는 관찰되지 않았다 —
  D7에서 의도적으로 미룬 항목이라, 실제 운영에서 관찰되면 후속 조건을 추가한다.

---

### P27. 배터리 3단계 재정렬 + 엘리베이터를 배타적 자원으로 ✅ 완료 (2026-08-25)

> 세부 설계·결정 근거(D1~D5): [`docs/p27-battery-tiers-elevator-queue-design.md`](./p27-battery-tiers-elevator-queue-design.md)

> 둘 다 "없는 데이터를 지어내는" 방향이 아니라 "이미 아는 물리적 제약을 강제하는"
> 방향으로 범위를 좁혔다 — 사양서의 엘리베이터 12단계 핸드셰이크(문 열림 유지 명령 등)를
> 문자 그대로 흉내 내려면 없는 도어 센서 신호를 지어내야 한다. 대신 그 프로세스가
> 실제로 강제하는 사실(엘리베이터는 한 번에 한 대만 태운다)만 가져왔다.

목표

- 배터리 임계값을 사양서 3단계(상 80%·중 30~80%·하 30% 미만)에 맞춰 재정렬 — 배차 최소
  25%→50%("50% 이상이어야 신규 작업 할당"), robot-sim 자가충전 트리거를 30%(낮으면
  충전) → **80% 미만이면 유휴 시 충전**으로 뒤집었다. 기존 불변식(충전 트리거 > 배차
  최소)은 80 > 50으로 그대로 유지 — `BatteryDeadZoneInvariantTest`가 이 관계를 소스에서
  직접 파싱해 계속 지킨다.
- `ElevatorController` 신설(`TrafficController`와 같은 패턴, 인메모리) — 층을 넘는
  두 운송이 겹치면 두 번째가 첫 번째 도착 이후로 큐잉된다. "잡았다 놓는다"가 아니라
  왕복 시간이 고정값으로 알려져 있어 **예약 시점에 큐 전체를 시간 계산**으로 처리한다
  (별도 점유/반납 생명주기가 필요 없다).

완료 기준 — 전부 실기동으로 확인(2026-08-25, factory+fleet+robot-sim 풀스택, 로봇 14대)

- [x] 배터리 49%로 떨어진 로봇이 유휴 상태에서 즉시 `CHARGING`으로 전환(신규 작업 배차
      제외, 사각지대 없이 스스로 충전 이동) — 실측 확인
- [x] 80~81% 로봇은 유휴 상태에서 충전하지 않고 대기 — 실측 확인(상위 등급 규칙)
- [x] 엘리베이터 이벤트가 실제로 발생하고(자연 트래픽, `TASK_CREATED` 로그) 예외 0건
- [x] `ElevatorController` 큐잉 로직 자체는 전용 단위 테스트 3건(무큐잉·겹침 큐잉·큐
      해소 후 재예약)으로 정확성 확인 — 실기동에서는 자연 발생한 두 운송이 12초 창 안에
      겹치는 사례를 관찰하지 못해(빈도가 낮다) 실측으로는 "정상 동작(비큐잉)"까지만
      확인, 큐잉 자체는 단위 테스트가 근거
- [x] 양쪽 모듈 컴파일·전체 테스트(`BatteryDeadZoneInvariantTest` 포함) 회귀 없음

---

### P36. 수집 계층 승격 — UNS 토픽 계약 · 생사 인증서 · 시계열 저장 📋 설계 초안 (2026-09-14)

> 세부 설계·결정 근거(D1~D9): [`docs/p36-uns-sparkplug-timeseries-design.md`](./p36-uns-sparkplug-timeseries-design.md)

> **수집 자체는 P8에서 이미 풀렸다.** 남은 것은 그 뒤 셋이다 — ① 태그에 의미를 붙이는
> 컨텍스트 모델링, ② 초당 수천 건을 받아내는 저장 모델, ③ 네트워크가 끊겼을 때의 유실
> 방지. 이 레포는 ②의 부하를 겪고 있지 않고(실측 약 3 events/s), 그 사실을 설계 문서
> 0-A절에 먼저 적었다 — 측정하지 않은 규모를 근거로 기술을 고르지 않는다.

목표

- **토픽에 의미를 붙인다(D1·D2).** 루트가 모듈 이름(`factory/`, `fleet/`)인 지금 구조는
  물리 세계에 없는 경계다. ISA-95 계층 `pixel/{site}/{area}/{line}/{cell}/{kind}`로
  통일하고, 물리 대상이 없는 업무 이벤트는 `pixel/_biz/...`로 분리한다. 지금은
  "1동 가공구역의 모든 것"을 구독할 방법이 아예 없다 — 구역 정보가 토픽에 없다.
- **발행자의 생사와 연속성을 계약으로 만든다(D3·D4·D8).** death는 LWT로 이미 구현돼
  있고 birth만 없다. Sparkplug B에서 **birth/death와 `seq`만 개념으로 차용**하고
  Protobuf 페이로드는 채택하지 않는다 — 초당 3건에서 대역폭 편익이 없고, 사람이 읽는
  `mqtt-topics.md` 계약과 `mosquitto_sub` 디버깅 편의를 잃는다. 그래서 "Sparkplug B
  호환"이라고 부르지 않는다.
- **유실을 관측 가능하게 만든다(D4·D5).** 지금 발행 측에는 유실 방어가 없다 —
  구독 측은 `cleanSession=false`+QoS 1로 방어돼 있는데 발행 측은 `MemoryPersistence` +
  `catch { System.err.println }`이라 브로커가 끊기면 그 구간 사이클이 영구 소실된다.
  비대칭이다. 게다가 **몇 건이 사라졌는지 셀 방법이 없어서 고쳐도 고쳤다고 말할 근거가
  없다** — 그래서 `seq` 기반 gap 카운터가 store-and-forward보다 먼저다.
- **이벤트 테이블을 시계열 저장소로 바꾼다(D6).** TimescaleDB 하이퍼테이블. 근거는
  처리량이 아니라 ⓐ 직접 짠 벌크 DELETE 잡(`FactoryEventRetentionJob`)이 보존 정책
  한 줄로 **삭제되고**, ⓑ 곧 직접 짜게 될 집계 캐시 배치를 연속 집계가 대체하며,
  ⓒ 부하가 없을 때 바꾸는 비용이 가장 싸다는 것.

완료 기준 (착수 전 — 전부 관측 가능한 값으로 잡았다)

- [ ] WP0 — 브로커를 30초 내렸다 올린 뒤 `factory_telemetry_gap_total`이 0이 아닌 값을
      가리킨다 (지금까지 셀 수 없던 유실이 숫자로 나온다)
- [ ] WP1 — `oee-service`만 재기동해도 설비 8대의 birth를 전부 받는다
- [ ] WP2 — **WP0과 똑같은 실험에서 gap이 0이 된다** (같은 실험, 다른 결과가 증거다)
- [ ] WP3 — `pixel/plant1/prod-a/#` 한 줄로 가공구역 설비 4대가 전부 잡히고,
      `pixel/plant1/#`에 업무 이벤트가 섞이지 않는다. 전환 4단계 내내 적재가 안 끊긴다
- [ ] WP4 — `FactoryEventRetentionJob`과 그 테스트가 **삭제된 상태로** 보존이 동작하고,
      시프트 OEE가 연속 집계를 타면서 원본과 값이 일치한다

주의

- **ACL이 현재 토픽 모양에 직접 묶여 있다.** `acl.conf`의 `topic write factory/#`를
  같은 커밋에서 안 바꾸면 조용히 전부 거부된다 — 거부는 클라이언트에 에러를 주지 않고
  메시지가 그냥 사라진다. 그래서 D7 브리지에서 ACL 추가를 **독립 단계**로 뺐다.
- **Railway Postgres 플러그인의 `timescaledb` 확장 지원 여부가 미확인이다.** 미지원이면
  WP4는 Postgres 이미지를 직접 띄우는 서비스로 바꾸는 작업(데이터 이관 포함)이 선행된다.
  확인 전에는 WP4 공수를 추정하지 않는다.
- 범위 밖으로 **명시적으로** 뺀 것: OPC UA(Milo)·Modbus·Sparkplug 규격 채택(Tahu)·
  Kafka 브리징·InfluxDB/ClickHouse·Grafana 별도 스택. 이유는 설계 문서 6절에 항목별로
  적었다 — 특히 OPC UA는 [1] 디바이스 계층이 없는 상태에서 붙이면 **변환되는 프로토콜이
  없는 가짜 계층**이 하나 느는 것뿐이다.
