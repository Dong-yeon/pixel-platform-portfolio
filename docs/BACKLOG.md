...
**지금은 충돌 회피가 전혀 없다.** 로봇들은 서로를 통과해 지나간다.

- ✅ 정차 겹침은 해결 — 노드 중심이 아니라 로봇별로 다른 자리에 선다(Simulator#spot).
- ✅ 주행 경로는 통로를 따른다 — 대각선으로 설비를 관통하지 않는다(NodeMap#route).
- ❌ **통로 위에서 서로 겹치는 것은 그대로다.** 같은 통로를 반대 방향으로 지나가면 통과한다.

실제 FMS는 경로 예약(segment reservation), 교차점 통제, 차간거리 유지로 푼다.
구현하려면 관제 서버가 경로를 소유하고 로봇에 구간 점유권을 주는 구조가 필요하다 —
지금은 로봇이 스스로 경로를 만들기 때문에 서버가 개입할 지점이 없다.
