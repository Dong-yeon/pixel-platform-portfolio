...
> ## ⚠️ 현재 이 배포는 내려가 있습니다 (2026-09-14)
>
> **2026-08-25 ~ 09-14 동안 아래 구성 그대로 운영했고, 비용 때문에 프로젝트를 삭제했습니다.**
> 컨테이너 10개(서비스 8 + Postgres + Redis)가 24시간 떠 있는 것이 개인 포트폴리오로는
> 과했습니다 — 특히 시뮬레이터 2개가 쉬지 않고 발행해 idle로 내려가는 구간이 없었습니다.
> 아래 절차와 실측 로그는 **실제로 돌려본 기록**이라 그대로 남겨 둡니다. 다시 올릴 때 이
> 문서대로 하면 됩니다.
>
> **다시 올릴 때 주의할 것 (실제로 겪은 순서대로):**
> - 서비스를 지워도 **볼륨은 같이 지워지지 않는다.** 삭제 후에도 `postgres-volume`(4.4GB)
>   등 4개가 `detached`로 남아 계속 스토리지 과금됐다. 프로젝트를 통째로 지워야 정리된다.
> - 프로젝트를 지우면 **환경변수가 같이 사라진다.** `PLATFORM_JWT_SECRET`과 §1의
>   fleet·wms·qms 롤 비밀번호는 이 문서에 플레이스홀더로만 적혀 있어 복구할 수 없다.
>   **지우기 전에 `railway variable list --service <이름> --json`으로 전부 빼둘 것**
>   (레포 밖에 저장 — 평문 시크릿이다).
> - 비용을 줄이면서 데모를 살리고 싶다면, 전체를 내리기 전에 **시뮬레이터 2개
>   (`factory-sim`·`robot-sim`)만 끄고 나머지에 App Sleeping을 켜는** 중간 단계를 먼저
>   시도해 볼 것. 다만 이 구성에서는 MQTT 상시 연결 때문에 실제로 잠들지 않을 가능성이
>   높아, 확인 없이 기대하지는 말 것.
>
> ---
>
> **(운영 당시) 배포 주소: `https://happyeon-pixel-platform.up.railway.app`**(2026-08-25부터).
...
>
> **`SIM_SPEED`를 넣지 말 것.** 배속을 올리면 발행량만 늘고 표준CT(설비 마스터)는 그대로라
> **OEE의 P가 그 배수만큼 부풀려진다**(예전에 10으로 두어 P가 250~650%로 나왔다).
> 사용량을 줄이려면 이 서비스를 꺼 둔다(설비가 마지막 상태로 남는다 — retained).
