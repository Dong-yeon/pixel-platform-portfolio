...
## 0-A. 실제 운영 시스템에서 가져온 근거

동연님이 실제로 운영·유지보수하는 여러 MES/WMS/QMS 시스템에서 확인한 것들이다(고객사명·
실제 테이블명은 공개 버전에서 일반화). **데모를 그럴듯하게 만드는 게 아니라, 실제 시스템이
그렇게 생긴 이유를 따라가는 것**이 목적이다.

### 도메인 구조 (P13·P14 설계 근거)

| 개념 | 실제 시스템 | 데모에 반영할 것 |
|---|---|---|
| **파렛트** | 파렛트 마스터/적재 테이블 + 파렛트 번호 — **1급 엔티티** | **결정: 엔터티로 만든다.** 운송 작업이 파렛트를 지정하고 지도·목록에 번호가 나온다. 지금의 `laden` 불리언은 1단계였고, LOT 추적(4단 트리)의 기반이 된다 → **P12 직후 착수**(fleet 쪽이라 P12와 독립) |
| **창고 도면** | 선반(rack) × 단(level 1~5) × 자리(bin). 실제 화면은 선반 15개 · 자리 2,631개 | **공장 평면도와 다른 좌표계다.** 별도 뷰여야 한다 |
| **자재 박스** | 박스 바코드, Box 잔량, 역순 차감/복원 | 자재 소비·반납을 이력으로 |
...
- [x] 배포 URL에서 데모 계정으로 전체 사이클이 관찰된다 (2026-09-02, 라이브 브라우저 확인)
- [x] 시연용 이벤트 주입 버튼이 동작한다 (P15-1 `ScenarioView`, 라이브 확인)
- [x] 24시간 방치 후에도 DB 용량이 통제된다 (`FactoryEventRetentionJob`/`FleetEventRetentionJob`, 90일 보존 + 매일 새벽 크론)
- [x] 익명 MQTT 접속이 거부된다 (2026-09-02, `allow_anonymous false` + password file + ACL, 6개 서비스 모두 인증 접속 라이브 확인)
...
| `railway domain --service X`를 "조회"로 착각 | 인자 없이 실행하면 **도메인을 실제로 만든다**(2026-09-02, mosquitto에 실수로 퍼블릭 도메인 생성). 목록만 보려면 `railway domain list --service X` |
