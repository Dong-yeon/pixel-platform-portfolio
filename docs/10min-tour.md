# 10분 투어 — 화면만 따라가며 파악하기

이 프로젝트가 뭘 하는지 코드를 열지 않고 10분 안에 파악하는 경로입니다.
`admin` / `password`로 로그인하세요(화면에 미리 채워져 있습니다).

> **상시 배포는 2026-09-14에 내려뒀습니다**(비용 — README 참고). 아래 스크린샷은 전부
> 실제 구동 화면이고, 직접 눌러 보시려면 README의 **Quick Start**로 로컬에서 띄우면
> 같은 화면이 그대로 나옵니다:
>
> ```powershell
> .\scripts\dev-up.ps1 -Stack full -Demo   # → http://localhost:9200
> ```
>
> 이 문서에 적힌 "몇 초 기다리면 숫자가 바뀝니다" 같은 관찰은 **시뮬레이터가 함께 떠
> 있어야** 성립합니다. `-Stack full -Demo`가 그걸 포함합니다.
...
> **오래 띄워 둔 인스턴스에서는 안 뜰 수도 있습니다.** 이 신호는 작업지시가 아직 계획 수량을
> 채우기 전 "진행 중"일 때만 나갑니다(`EquipmentTelemetryService.recordCycle`). 인스턴스가
