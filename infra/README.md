...
| 서비스 | 포트 | 용도 |
|---|---|---|
| `pixel-postgres` | 5432 | 모듈당 DB 하나 (`pixelfactory`, `pixelfleet`, `pixelwms`) |
...
**모듈 간 통지**: fleet이 운송 작업 종료를 `fleet/tasks/{taskCode}/{completed\|failed}`로 발행한다.
로봇 텔레메트리(3마디)와 마디 수가 달라 fleet 자신의 `fleet/#` 핸들러는 이를 무시하고,
관심 있는 모듈(WMS)만 구독한다 — **fleet은 구독자를 모른다**(컴포저블).
...
> 볼륨이 이미 있으면 `postgres-init`이 다시 돌지 않아 `pixelwms`가 없다. 볼륨을 지우지 않고
> 추가하려면:
> ```powershell
