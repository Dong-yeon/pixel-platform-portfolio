...
- **OEE는 서버가 계산한 값을 그대로 쓴다.** 대시보드에서 다시 계산하지 않는다.
  ...
  이벤트마다 계산하면 DB만 두드린다).
- `modules/pixel-fleet/web/`은 이 대시보드로 흡수되었다(D3 결정, `docs/pixel-platform-plan.md`).
  당시엔 모듈 단독 개발용으로 남겨뒀지만, 갱신 없이 방치돼 있던 걸 정리하며 삭제했다
  (2026-09) — 플랫폼에서는 처음부터 이 대시보드만 썼고, 모듈 단독 개발이 다시 필요하면
  이 화면 코드를 그대로 재사용하면 된다.
