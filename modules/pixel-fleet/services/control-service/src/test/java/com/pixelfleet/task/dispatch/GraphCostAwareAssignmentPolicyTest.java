...
class GraphCostAwareAssignmentPolicyTest {
    ...
    void 장애물이_있으면_직선최근접과_그래프비용_최적이_서로_다른_로봇을_고른다() {
        // 작업 출발지: PROD-B1(62,21) — P30에서 창고동에 4번째 베이가 생긴 만큼(+13)
        // 생산동도 옮겨졌다(코드는 그대로, 실제 x만 62/69로 바뀜). 후보 둘 다 생산동
        // 상단(A열)에 있다.
        //   - AMR-NEAR = PROD-A1(62,6) — PROD-B1과 같은 연결로. 직선거리 15로 더 가깝다.
        //   - AMR-FAR  = PROD-A2(69,6) — 직선거리 √274≈16.55로 더 멀다.
        // 연결로(코드는 JCT-27, 실제 x=62)의 중간 대역을 막으면 AMR-NEAR는 연결로
        // 69(코드 JCT-34)를 거쳐 돌아가야 한다(비용 29) — 반면 AMR-FAR는 애초에
        ...
        RobotResponse near = robot(1L, "AMR-NEAR", 62, 6);
        RobotResponse far = robot(2L, "AMR-FAR", 69, 6);
