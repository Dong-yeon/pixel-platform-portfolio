...
/**
 * 창고동 렉 코드 사본 — factory {@code layout_racks}와 같은 912기(1층 864 + 2·3층 각 24,
 * P32로 1층만 전면 재발번).
 *
 ...
 *
 * <p><b>왜 1층은 리스트가 아니라 생성 루프인가(P32).</b> 864개를 문자열로 나열하면 파일
 * 하나가 화면 밖으로 넘어가고, 손으로 옮겨 적다 하나라도 틀리면 조용히 어긋난다. factory
 * {@code V22__warehouse_band_relayout.sql}이 쓰는 것과 <b>정확히 같은 공식</b>
 * (밴드 12개 × 열 72 — 열 1~36이 앞줄, 37~72가 뒷줄)으로 코드만 재생성한다(좌표는 이
 * 클래스가 안 가지므로 코드 문자열만 맞으면 된다). 공식이 바뀌면 두 곳(factory 마이그레이션·
 * 여기)을 같이 고쳐야 하고, {@code RackMapLayoutConsistencyTest}가 어긋남을 잡는다.
 *
 ...
 * 완료된다) — {@code RackMapLayoutConsistencyTest}가 factory V12+V19+V21+V22 마이그레이션과
 * 대조한다.
 */
@Component
